...
const ThirdBlock = ({mascotas}) => {
    ...
    return (
        <div className='mt-4 block md:grid md:grid-cols-4 md:justify-between md:gap-x-16 md:gap-y-2'>
            {
                mascotas.map(m => (
                    <div key={m.id} className='w-[330px] h-[215px] mt-4 flex justify-center flex-col text-center rounded-xl md:w-[330px] md:h-[215px]   bg-white mx-auto'>
                        <img src={m.urlFotoPerfil} alt="fotoperro" className='w-12 h-12 mx-auto rounded-full' />
                        <p>{m.nombre}</p>
                        <p>{m.especie}</p>
